#!/usr/bin/env python3
"""EcoBloom demo launcher.

This convenience script starts a small local P2P network (3 peers) and
opens the dashboard for demonstration. It's intentionally simple and
suitable for workshop/demos — use the `single` mode for a quick local test.
"""
...
def start_farm(farm_id, host, port, bootstrap_peers=None):
    """Start a single farm peer process and optionally connect to bootstraps.

    farm_id is used only for human-readable logging and to optionally start
    the WebSocket bridge on Farm A.
    """
    print(f"\n{'='*50}")

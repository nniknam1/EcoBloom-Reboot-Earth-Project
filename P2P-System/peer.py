...
class Peer:
    ...
    def load_or_create_peer_id(self):
        """Load an existing peer id from disk or create a new one.

        The id file lives in the local `ids/` directory and uses the host
        and port to avoid collisions when running multiple peers locally.
    ...
    def create_listening_socket(self):
        """Open a non-blocking TCP listening socket and register it with the selector.

        The socket uses SO_REUSEADDR to make restarts during development easier.
        """
    ...
    def start_listening(self):
        """Main selector loop: accept new connections and service existing ones.

        Runs in a background thread and uses a 1s select timeout so the loop
        can check for shutdown signals regularly.
        """
        try:
    ...
    def handle_message(self, message, connection):
        # Prevent message storms by deduplicating on message_id
    ...
    def handle_handshake(self, message, connection):
        """Process an incoming handshake and exchange peer information.

        After the handshake completes we update local known_peers and
        broadcast updated peer lists and network topology to connected peers.
        """
    ...
    def queue_offline_message(self, message):
        """Persist a message for later delivery when the target becomes reachable.

        Currently this stores the message in the peer's local MessageStore.
        A retry/scheduler mechanism can later read from the store and attempt
        retransmission.
        """

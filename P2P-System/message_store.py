...
class MessageStore:
    """Persistent store for messages that couldn't be delivered immediately.

    Each peer has its own SQLite database under `database/`. Messages are
    stored in `offline_messages` and a companion `schedule_messages` table
    tracks retry metadata and expiry times.
    """
    ...
    def get_pending_messages(self, target_peer):
        ...
        messages = [self.data_to_message_class(r) for r in result_raw]

        # Once fetched, remove messages that were delivered to avoid duplicates
